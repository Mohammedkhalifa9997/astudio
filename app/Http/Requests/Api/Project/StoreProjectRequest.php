...
class StoreProjectRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'attributes.*' => ['required', 'integer', Rule::exists('attributes', 'id')],

            'values' => ['required', 'array', 'min:1'],
            'values.*' => ['required', 'string', 'max:255'],

            'users' => ['nullable', 'sometimes', 'array', 'min:1'],
        ...
        $validator->after(function ($validator) {
            ...
            foreach ($attributes as $index => $attributeId) {
                ...
                switch ($attribute->type->value) {
                    ...
                    case 'select':
                        if (!is_string($value) || empty(trim($value))) {
                            $validator->errors()->add("values.$index", __('The value must be a valid text.'));
                        } else {
